...
class GaiaxParticipantClaimsEvaluationFunctionTest {
/*
    private static final ObjectMapper MAPPER = mock(ObjectMapper.class);
    private static final Monitor MONITOR = mock(Monitor.class);

    private GaiaxParticipantClaimsEvaluationFunction function;

    @BeforeEach
    public void setUp() {
        function = new GaiaxParticipantClaimsEvaluationFunction(MONITOR, MAPPER, null);
    }

    @Test
    void verifyGetClaimsShapeType() {
        assertThat(function.getClaimsShapeType()).isEqualTo(GaiaxParticipantClaims.class);
    }
